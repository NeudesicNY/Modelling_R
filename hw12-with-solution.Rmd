---
title: "CSCI E-63C Week 12 assignment: solution"
output:
  html_document:
    toc: true
---
...
plotNNpreds2D2class <- function(inpNN,inpClassThresh,inpGrid=(-60:60)/10) {
  ...
  plot(xyGridTmp,col=as.numeric(gridValsTmp$net.result>inpClassThresh)+1,pch=20,cex=0.3,main=paste("Error:",signif(mean(inpNN$err.fct(inpNN$response,inpNN$net.result[[1]][,1])),6)))
...
```{r}
make3DconcSph2classDat <- function(inpNobs,inpThresh=1.5,inpNnull=0) {
  xyzTmp <- matrix(rnorm(3*inpNobs),ncol=3)
  clTmp <- as.numeric(factor(sqrt(rowSums(xyzTmp^2)) < inpThresh))
  if ( inpNnull > 0 ) {
    xyzTmp <- cbind(xyzTmp,matrix(rnorm(inpNnull*inpNobs),ncol=inpNnull))
  }
  list(xyz=xyzTmp,class=clTmp)
}
lXYZtmp <- make3DconcSph2classDat(1000)
pairs(lXYZtmp$xyz,col=lXYZtmp$class,pch=lXYZtmp$class)
table(lXYZtmp$class)
```

The plots above show that points belonging to one of the two classes form a three-dimensional ball surrounded by the points from the second class.  The observations in the entire dataset are approximately evenly split between these two classes.  The closer the decision surface of the classifier approximates true spherical boundary between these two classes the smaller will be the error.  As a very rough approximation a tetrahedron-like shape made of four planes could enclose all points from the inner class. This would suggest that a neural network with four nodes in the hidden layer could have noticeably better performance than those with fewer nodes.
...
for ( iHidden in 1:6 ) {
  for ( iTry in 1:3 ) {
    lXYZtmp <- make3DconcSph2classDat(1000)
    nnResi <- neuralnet(g~X1+X2+X3,data.frame(g=lXYZtmp$class,lXYZtmp$xyz),hidden=iHidden)
    if ( iTry == 1 ) {
      plot(nnResi$net.result[[1]][,1],col=lXYZtmp$class,pch=lXYZtmp$class,main=paste(iHidden,"hidden nodes"))
    }
    lXYZtmpTest <- make3DconcSph2classDat(10000)
    tmpXYZpred <- compute(nnResi,lXYZtmpTest$xyz)
    #plot(nnResi)
    trainTblTmp <- table(nnResi$response,nnResi$net.result[[1]][,1]>1.5)
    testTblTmp <- table(lXYZtmpTest$class,tmpXYZpred$net.result[,1]>1.5)
    dfTmp <- rbind(dfTmp,data.frame(k=iHidden,type=c("train","test"),err=c(mean(nnResi$err.fct(nnResi$response,nnResi$net.result[[1]][,1])),mean(nnResi$err.fct(tmpXYZpred$net.result[,1],lXYZtmpTest$class))),acc=c(sum(diag(trainTblTmp))/sum(trainTblTmp),sum(diag(testTblTmp))/sum(testTblTmp))))
    #print(tail(dfTmp))
  }
}
par(old.par)
ggplot(dfTmp,aes(x=k,y=err,colour=type,fill=type))+geom_point(position=position_jitterdodge())
ggplot(dfTmp,aes(x=k,y=1-acc,colour=type,fill=type))+geom_point(position=position_jitterdodge())
```

The plots above show that with the increase in the number of hidden nodes response values estimated by neural network become progressively more distinct between the observations belonging to each of the two classes.  The biggest decrease in the training and test errors (that closely follow each other) occurs for going from one to four hidden nodes and for four, five and six nodes test and training errors remain approximately the constant.  This is in agreement with the geometrical considerations in this simulation where it takes at least four planes in three-dimensional space to fully enclose observations from the inner class.
...
```{r}
dfTmp <- NULL
for ( iNull in c(0,1,2,5) ) {
  for ( iSmplSz in c(100,200,500) ) {
    for ( iHidden in 1:6 ) {
      for ( iTry in 1:10 ) {
        lXYZtmp <- make3DconcSph2classDat(inpNobs=iSmplSz,inpNnull=iNull)
        nnResi <- NULL
        # when neuralnet does not converge, there is no result.matrix:
        while ( ! "result.matrix" %in% names(nnResi) ) {
          nnResi <- neuralnet(as.formula(paste0("g~",paste0("X",1:(3+iNull),collapse="+"))),data.frame(g=lXYZtmp$class,lXYZtmp$xyz),hidden=iHidden)
        }
        #plot(nnResi$net.result[[1]][,1],col=lXYZtmp$class,pch=lXYZtmp$class)
        lXYZtmpTest <- make3DconcSph2classDat(10000,inpNnull=iNull)
        tmpXYZpred <- compute(nnResi,lXYZtmpTest$xyz)
        #plot(nnResi)
        dfTmp <- rbind(dfTmp,data.frame(nNull=iNull,nObs=iSmplSz,k=iHidden,type=c("train","test"),err=c(mean(nnResi$err.fct(nnResi$response,nnResi$net.result[[1]][,1])),mean(nnResi$err.fct(tmpXYZpred$net.result[,1],lXYZtmpTest$class)))))
        #print(tail(dfTmp))
      }
    }
  }
}
ggplot(dfTmp,aes(x=k,y=err,colour=type,shape=factor(nNull)))+geom_point(position=position_jitterdodge())+facet_wrap(~nObs)
```

The plots above illustrate that for smaller samples sizes `neuralnet` tends to more profoundly overfit and the increase in the model complexity (number of hidden nodes) has lesser impact on the test error of the model.  The increase in the number of null variables tends to further increase test error.
...
for ( iHidden in 1:5 ) {
  for ( iTry in 1:10 ) {
    trainIdx <- sample(nrow(dbaDat),nrow(dbaDat)*2/3)
    dbaNNres <- NULL
    while ( ! "result.matrix" %in% names(dbaNNres) ) {
      dbaNNres <- neuralnet(as.numeric(auth)~var+skew+curt+entr,dbaDat[trainIdx,],hidden=iHidden)
    }
    dbaNNpred <- compute(dbaNNres,dbaDat[-trainIdx,colnames(dbaDat)!="auth"])
    #plot(dbaNNpred$net.result[,1],col=as.numeric(dbaDat[-trainIdx,"auth"]))
    tblTmpTrain <- table(as.numeric(dbaDat[trainIdx,"auth"]),1+as.numeric(dbaNNres$net.result[[1]][,1]>1.5))
    tblTmp <- table(as.numeric(dbaDat[-trainIdx,"auth"]),1+as.numeric(dbaNNpred$net.result[,1]>1.5))
    #print(tblTmp)
    #cat(iHidden,iTry,1-sum(diag(tblTmp))/sum(tblTmp),1-sum(diag(tblTmpTrain))/sum(tblTmpTrain),fill=TRUE)
    dfTmp <- rbind(dfTmp,data.frame(hidden=iHidden,type=c("test","train"),error=c(1-sum(diag(tblTmp))/sum(tblTmp),1-sum(diag(tblTmpTrain))/sum(tblTmpTrain))))
  }
}
ggplot(dfTmp,aes(x=factor(hidden),y=error,colour=type))+geom_point(position=position_jitterdodge())
```

The plot above shows that the test error of the neural network (with all default parameters) on banknote authentication dataset is around 1% or less (smaller for higher number of hidden nodes) that is approximately comparable to that observed  (in week 9 and 11 assignments) for logistic regression/LDA/QDA/random forest while SVM and KNN for smaller numbers of nearest neighbors achieve even lower error (often near zero).  These lower error rates are approached by neural network with 4-5 nodes in hidden layer.  For these conditions on this dataset the training and test errors are approximately comparable.